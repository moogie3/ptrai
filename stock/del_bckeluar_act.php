...
try {
    ...
    // Retrieve the id of the last inserted record from sbarangckeluar table
    $result = mysqli_query($conn, "SELECT MAX(id) AS id FROM sbarangckeluar");
    if ($row = mysqli_fetch_assoc($result)) {
        $lastId = $row['id'];

        // Fetch the information of the last inserted record before deleting it
        $query = mysqli_query($conn, "SELECT * FROM sbarangckeluar WHERE id = $lastId");
        if ($data = mysqli_fetch_assoc($query)) {
            $kodebarang = $data['idx'];
            $keluar = $data['keluar'];

            // Delete the last inserted record from sbarangckeluar
            $queryDelete = mysqli_query($conn, "DELETE FROM sbarangckeluar WHERE id = $lastId");
            if ($queryDelete) {
                // Revert the "keluar" value in stockc table
                $queryUpdate = mysqli_query($conn, "UPDATE stockc SET jkeluar = jkeluar - $keluar WHERE idx = '$kodebarang'");
                if ($queryUpdate) {
                    ...
                    echo "<div class='alert alert-success'>
                        ...
                        <meta http-equiv='refresh' content='1; url= consumablekeluar.php'/>";
                } else {
