...
try {
    ...
    $result = mysqli_query($conn, "SELECT MAX(id) AS id FROM sbarangmmasuk");
    if ($row = mysqli_fetch_assoc($result)) {
        $lastId = $row['id'];

        // Fetch the information of the last inserted record before deleting it
        $query = mysqli_query($conn, "SELECT * FROM sbarangmmasuk WHERE id = $lastId");
        if ($data = mysqli_fetch_assoc($query)) {
            $kodebarang = $data['idx'];
            $masuk = $data['masuk'];

            // Delete the last inserted record from sbarangcmasuk
            $queryDelete = mysqli_query($conn, "DELETE FROM sbarangmmasuk WHERE id = $lastId");
            if ($queryDelete) {
                // Revert the "masuk" value in stockc table
                $queryUpdate = mysqli_query($conn, "UPDATE stockm SET jmasuk = jmasuk - $masuk WHERE idx = '$kodebarang'");
                if ($queryUpdate) {
                    ...
                    echo "<div class='alert alert-success'>
                        ...
                        <meta http-equiv='refresh' content='1; url= materialmasuk.php'/>";
                } else {
